...
namespace algorithmcpp {
	...
	public:
		AcyclicSP() = delete;
		AcyclicSP(const AcyclicSP&) = default;
		AcyclicSP(AcyclicSP &&) noexcept = default;
		AcyclicSP &operator=(const AcyclicSP &) = default;
		AcyclicSP &operator=(AcyclicSP &&) noexcept = default;
		~AcyclicSP() = default;

		AcyclicSP(const EdgeWeightedDigraph &G, size_t s) :
			dist_to_(std::vector<double>(G.V(),std::numeric_limits<double>::max())), edge_to_(std::vector<DirectedEdge>(G.V())) {
			ValidateVertex(s);
			dist_to_[s] = 0.0;
